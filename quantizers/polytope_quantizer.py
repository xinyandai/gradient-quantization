...
class PolytopeQuantizer(BaseQuantizer):
...
class PolytopeCompressor(object):
    def __init__(self, size, shape, c_dim=128, ks=128):
        ...
        self.shape = shape
        self.dim = c_dim
        self.code_dtype = np.uint8 if self.Ks <= 2 ** 7 else (
            np.uint16 if self.Ks <= 2 ** 15 else np.uint32)
    ...
    def compress(self, vec):
        ...
        sign_flip = (
            1 - np.sign(vec[np.arange(len(vec)), codes]).astype(dtype=np.int32)) * self.dim // 2
